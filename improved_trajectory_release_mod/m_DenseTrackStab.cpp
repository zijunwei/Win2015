/* Modify the original code to take input as a directory of frames instead of a video
...
int main(int argc, char** argv)
{
	...
	SeqInfo seqInfo;
	InitSeqInfo(&seqInfo, frmPattern);
    ...
    sprintf(frameFile, frmPattern, start_frame);        
	Mat frame = imread(frameFile, CV_LOAD_IMAGE_COLOR);
	if(frame.empty()) {
	...
	if(bb_file)
		InitMaskWithBox(human_mask, bb_list[frameNum].BBs);
	...
	for (frameNum = start_frame+1; frameNum <= end_frame; frameNum++){
		//fprintf(stderr, "Processing frameNum = %d\n", frameNum);
        int i, c;

        sprintf(frameFile, frmPattern, frameNum);        
    	frame = imread(frameFile, CV_LOAD_IMAGE_COLOR);
		...
		// match surf features
		if(bb_file)
			InitMaskWithBox(human_mask, bb_list[frameNum-1].BBs);
			...
			for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
				...
				if(iTrack->index >= trackInfo.length) {
					...
					if(IsValid(trajectory, mean_x, mean_y, var_x, var_y, length) && IsCameraMotion(displacement)) {
						...
						printf("%d\t%f\t%f\t%f\t%f\t%f\t%f\t", frameNum, mean_x, mean_y, var_x, var_y, length, fscales[iScale]);

						// for spatio-temporal pyramid
						printf("%f\t", std::min<float>(std::max<float>(mean_x/float(seqInfo.width), 0), 0.999));
						printf("%f\t", std::min<float>(std::max<float>(mean_y/float(seqInfo.height), 0), 0.999));
						printf("%f\t", std::min<float>(std::max<float>((frameNum - trackInfo.length/2.0 - start_frame)/float(seqInfo.length), 0), 0.999));
					    //output the dense absolute trajectory
					    for (int i = 0; i < trackInfo.length; ++i){
