...
#include <sys/stat.h> // added by Minh
#include <libgen.h> // added by Minh for basename

using namespace cv;

// added by Minh
inline bool exist_file (char* name) {
  struct stat buffer;   
...
void InitSeqInfo(SeqInfo* seqInfo, char* frmPattern)
{
	char frameFile[1000];
	...
	for (frame_num = 1; ;frame_num++){
		sprintf(frameFile, frmPattern, frame_num);        
		if (!exist_file(frameFile)) break;
	}
...
void usage()
{
	...
	fprintf(stderr, "  -S [start frame]          The start frame to compute feature (default: S=1 frame)\n");
