...
void InitMaskWithBox(Mat& mask, std::vector<BoundBox>& bbs)
{
	...
	for(int k = 0; k < bbs.size(); k++) {
		BoundBox& bb = bbs[k];
		int y1 = max(min(cvCeil(bb.TopLeft.y),     height), 0);
		int y2 = max(min(cvCeil(bb.BottomRight.y), height), 0);
		int x1 = max(min(cvCeil(bb.TopLeft.x),     width ), 0);
		int x2 = max(min(cvCeil(bb.BottomRight.x), width ), 0);
		x2 = max(x2, x1);
		y2 = max(y2, y1);
