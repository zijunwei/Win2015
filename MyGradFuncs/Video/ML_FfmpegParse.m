...
% Last modified: 11-Jan-2014
    ...
    properties (Constant)
        ffprobeOnDesktop =   '/usr/local/bin/ffprobe';
        ffprobeOnMac =   '/usr/local/bin/ffprobe';
        ffprobeOnTitan = '/usr/local/bin/ffprobe';
    end
    ...
    methods (Static)
        ...
        function [videoStreamAttrs, audioStreamAttrs] = getAttrs(vidFile, ffprobeBin)
            if ~exist('ffprobeBin', 'var')
                ...
                if isempty(machine)
                    ffprobeBin = ML_FfmpegParse.ffprobeOnDesktop;
                elseif strcmpi(machine, 'titan')
                    ffprobeBin = ML_FfmpegParse.ffprobeOnTitan;
                end;
            end;
            
%             tmpDir = '/tmp/minhhoai/ML_FfmpegParse/';
            tmpDir = '/Users/zijunwei/tmp';
