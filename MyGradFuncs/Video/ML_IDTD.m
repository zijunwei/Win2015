classdef ML_IDTD
    ...
    properties (Constant)
        ...
        denseTrackBin = '/Users/zijunwei/Dev/Win2015/improved_trajectory_release_mod/release/m_DenseTrackStab';
        ...
        %         export_cmd = 'export LD_LIBRARY_PATH=/usr/local/lib/';
    ...
    methods (Static)
        ...
        function fv = fvEncode4pattern(frmPattern, nFrm, gmmModelFile)
           ...
           tmpDtdFilePrefix = fullfile( frmDir,   sprintf('dtdFeat_%s',  ml_randStr()));

%             tmpDtdFilePrefix = sprintf('%sdtdFeat_%s', frmDir, ml_randStr());
        ...
        function outFile = cmpDTD_helper(frmPattern, startIdx, endIdx, outFilePrefix, humanDetFile)
            ...
            cmd = sprintf('%s %s %s %s -L %d %s> %s', ...
                ML_IDTD.denseTrackBin, frmPattern, ...
                startOpt, endOpt, trajLen, humanDetOpt, sprintf('%s.txt', outFilePrefix));
            fprintf('%s\n', cmd);
            system(cmd);
