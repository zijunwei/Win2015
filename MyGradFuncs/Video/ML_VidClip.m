...
% Last modified: 11-Oct-2014    
    methods (Static)
        ...
        function frmPattern = extFrms(clip, outFrmDir, opts)
            ...
            ffmpegBin = ML_VidClip.parseOpts(opts, 'ffmpegBin', '/users/omkar/arnie/bin/ffmpeg');
            ...
            frmResolution = sprintf('-s %dx%d', newW, newH);
        ...
        function extVid(clip, outVidFile, opts)
            ...
            ffmpegBin = ML_VidClip.parseOpts(opts, 'ffmpegBin', '/Users/zijunwei/Dev/ffmpeg');
            fps = ML_VidClip.parseOpts(opts, 'fps', []);
            if isempty(fps)
                fpsOpt = '';
            else
