function bigIm = ml_data2Img_ws(data, h, w, ws, nC, nR, mode)
% bigIm = m_data2Img_ws(data, h, w, ws, nC, nR, mode)
% bigIm = m_data2Img_ws(data, h, w)
...
if exist('mode', 'var') && strcmp(mode, 'color')
    bigIm = 255*ones(nR*(h+ws), nC*(w+ws), 3);
    for i = 1:n;
        r = ceil(i/nC);
        c = mod(i-1, nC) + 1;        
        bigIm((r-1)*ws + ((1 + (r-1)*h):(r*h)), (c-1)*ws + ((1+ (c-1)*w):(c*w)), :) = ...
            reshape(data(:,i), h, w, 3);
    end; 
else
    bigIm = 255*ones(nR*(h+ws), nC*(w+ws));
    for i = 1:n;
        r = ceil(i/nC);
        c = mod(i-1, nC) + 1;        
        bigIm((r-1)*ws + ((1 + (r-1)*h):(r*h)), (c-1)*ws + ((1+ (c-1)*w):(c*w))) = ...
